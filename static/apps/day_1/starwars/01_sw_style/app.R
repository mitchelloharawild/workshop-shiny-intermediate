...
ui <- fluidPage(titlePanel("Starwars characters"), sidebarLayout(
  sidebarPanel(
    sliderInput("height", "Height: ", min = 0, max = max(starwars$height, na.rm = TRUE), value = c(0, max(starwars$height, na.rm = TRUE))),
      sliderInput(min = 0, "mass", "Mass: ", 
...
value = c(0, max(starwars$mass, na.rm = TRUE))),
      ...
      selectizeInput("Species:", inputId = "species",
  ...
  server <- function(input, output, session) {
    character_data <- reactive({
      starwars %>% filter(between(height, input$height[1], input$height[2]),
          between(mass, input$mass[1], input$mass[2])) %>% filter(trimws(format(gender)) %in% input$gender,
